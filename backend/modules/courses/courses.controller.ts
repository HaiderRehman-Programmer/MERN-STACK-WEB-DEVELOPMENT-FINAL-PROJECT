...
export const togglePublish = catchAsync(async (req: Request, res: Response) => {
  const id = req.params['id'] as string;
  const result = await CourseService.togglePublish(id, req.user!.id, req.user!.role);
  res.status(200).json({ success: true, data: result });
});

export const updateCourse = catchAsync(async (req: Request, res: Response) => {
  const id = req.params['id'] as string;
  const result = await CourseService.updateCourse(id, req.body, req.user!.id, req.user!.role);
  res.status(200).json({ success: true, data: result });
});
