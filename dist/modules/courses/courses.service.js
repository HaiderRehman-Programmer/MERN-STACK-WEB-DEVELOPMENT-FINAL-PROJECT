...
class CourseService {
    static async createCourse(data, instructorId) {
        ...
        try {
            ...
            if (error.code === 'SQLITE_CONSTRAINT_UNIQUE') {
                throw new AppError_1.AppError('A course with this exact title already exists. Please choose a unique name.', 400);
            }
            throw error;
        }
    ...
    static async findCourses(queryParams) {
        ...
        if (searchIds) {
            conditions.push((0, drizzle_orm_1.inArray)(schema_1.coursesTable.id, searchIds));
        }
        ...
        const coursesQuery = db_1.db
            ...
            avgRating: (0, drizzle_orm_1.sql) `round(cast(avg(${schema_1.reviewsTable.rating}) as real), 1)`,
    ...
    static async getInstructorAnalytics(instructorId) {
        ...
        // 1. Fetch individual course stats in one grouped query (Fixes N+1 issue)
        const coursesStats = await db_1.db.select({
            id: schema_1.coursesTable.id,
            title: schema_1.coursesTable.title,
            price: schema_1.coursesTable.price,
            isPublished: schema_1.coursesTable.isPublished,
            enrollments: (0, drizzle_orm_1.sql) `cast(count(distinct ${schema_1.enrollmentsTable.id}) as int)`,
            avgRating: (0, drizzle_orm_1.sql) `round(cast(avg(${schema_1.reviewsTable.rating}) as real), 1)`,
            reviewCount: (0, drizzle_orm_1.sql) `cast(count(distinct ${schema_1.reviewsTable.id}) as int)`,
        })
            .from(schema_1.coursesTable)
        ...
        const courseAnalytics = coursesStats.map(c => ({
            courseId: c.id,
            title: c.title,
            price: c.price,
            isPublished: c.isPublished,
            enrollments: c.enrollments,
            revenue: Number((c.enrollments * c.price).toFixed(2)),
            avgRating: Number(c.avgRating || 0),
            reviewCount: Number(c.reviewCount || 0),
        }));
        ...
        const sortKeyExpr = (0, drizzle_orm_1.sql) `strftime('%Y-%m', ${schema_1.enrollmentsTable.purchasedAt}, 'unixepoch')`;
        const trendResult = await db_1.db.select({
            sortKey: sortKeyExpr,
            enrollments: (0, drizzle_orm_1.sql) `cast(count(*) as int)`,
            revenue: (0, drizzle_orm_1.sql) `cast(sum(${schema_1.coursesTable.price}) as real)`
        })
            ...
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.sql) `${schema_1.enrollmentsTable.purchasedAt} >= ${Math.floor(sixMonthsAgo.getTime() / 1000)}`, (0, drizzle_orm_1.eq)(schema_1.coursesTable.instructorId, instructorId)))
            .groupBy(sortKeyExpr).orderBy(sortKeyExpr);
        ...
        const quizPerformance = await Promise.all(instructorQuizzes.map(async (quiz) => {
            const attempts = await db_1.db.select({
                ...
                passCount: (0, drizzle_orm_1.sql) `cast(sum(case when ${schema_1.quizAttemptsTable.passed} = 1 then 1 else 0 end) as int)`,
