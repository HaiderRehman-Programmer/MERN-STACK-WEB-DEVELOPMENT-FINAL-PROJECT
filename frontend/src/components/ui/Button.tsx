...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'manage' | 'enroll' | 'danger' | 'success';
  size?: 'sm' | 'md' | 'lg';
  isLoading?: boolean;
  isMagnetic?: boolean;
}
...
const Button: React.FC<ButtonProps> = ({ 
  ...
  const getCustomStyles = () => {
    if (variant === 'danger') return { background: 'rgba(255, 107, 107, 0.1)', color: '#ff6b6b' };
    if (variant === 'success') return { background: 'rgba(34, 197, 94, 0.1)', color: '#4ade80' };
    return {};
  };
