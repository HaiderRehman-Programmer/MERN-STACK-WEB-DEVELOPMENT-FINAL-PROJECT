...
import { Star, Globe, Lock, Heart } from 'lucide-react';
...
interface CourseCardProps {
  ...
  onToggleWishlist?: (isFavorited: boolean) => void;
}
...
const CourseCard: React.FC<CourseCardProps> = ({ 
  ...
  const handleToggleWishlist = async (e: React.MouseEvent) => {
    ...
    setIsToggling(true);
    try {
      const res = await api.post(`/wishlist/${course.id}`);
      const favorited = res.data.data.favorited;
      setIsFavorited(favorited);
      if (onToggleWishlist) onToggleWishlist(favorited);
    } catch (err) {
  ...
  return (
    <GlassPanel 
      ...
      style={style}
    >
      <div className="course-card-badges" style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem', alignItems: 'center' }}>
        ...
        <button 
          ...
          style={{
            ...
            cursor: 'pointer',
            padding: 0,
            color: isFavorited ? '#ef4444' : 'rgba(255,255,255,0.3)',
        ...
        >
          <Heart size={20} fill={isFavorited ? '#ef4444' : 'transparent'} />
        </button>
      </div>
