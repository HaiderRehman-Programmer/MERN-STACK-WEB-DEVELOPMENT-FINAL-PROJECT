...
import { User, BookOpen, Calendar, ExternalLink, Lock, Shield, Check, X, Edit3, Camera } from 'lucide-react';
import { Link } from 'react-router-dom';
import api, { ASSET_URL } from '../utils/api';
...
const Profile: React.FC = () => {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fetchProfile(); // Revert preview on failure
    } finally {
  ...
  const avatarSrc = displayUser?.avatarUrl?.startsWith('data:') 
    ? displayUser.avatarUrl 
    : displayUser?.avatarUrl 
      ? `${ASSET_URL}/${displayUser.avatarUrl}` 
      : null;
      ...
      <div className="glass-panel profile-card" style={{ marginBottom: '2rem' }}>
        <div className="profile-avatar" style={{ position: 'relative' }}>
          ...
          <label className="avatar-upload-overlay" style={{
            ...
            {isUploading ? <span className="loader" style={{ width: '14px', height: '14px' }} /> : <Camera size={14} color="white" />}
